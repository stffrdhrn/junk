...
#define FAIL_EXIT1(...) { fprintf(stderr, __VA_ARGS__); exit(1); }
...
#define M 10
  ...
  pthread_t th;
  void *thr_stack = NULL;
  posix_memalign (&thr_stack, 0x200, STACK_SIZE + 1);
  pthread_attr_setstack (&a, thr_stack + 1, STACK_SIZE);
